...
const Index = () => {
  ...
  const filteredHospitals = hospitals.filter(hospital => {
    // Search filter
    const matchesSearch = hospital.name.toLowerCase().includes(searchQuery.toLowerCase()) || hospital.address.toLowerCase().includes(searchQuery.toLowerCase());
    if (!matchesSearch) return false;

    // Location filter
    if (locationMode === "all") return true;
    if (locationMode === "nearby" && userLocation && hospital.latitude && hospital.longitude) {
      const distance = calculateDistance(userLocation.latitude, userLocation.longitude, hospital.latitude, hospital.longitude);
      ...
      <main className="container mx-auto px-4 py-12 bg-[#8888c7]">
        ...
        <div className="text-center mb-12 max-w-3xl mx-auto bg-card/50">
          ...
          <div className="relative max-w-xl mx-auto mb-6">
            ...
            <Input type="text" placeholder="Search by hospital name or location..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-12 h-14 text-lg shadow-soft" />
          </div>

          {/* Location Filter */}
          <div className="max-w-xl mx-auto">
            <div className="flex flex-wrap gap-3 justify-center mb-4">
              <Button variant={locationMode === "nearby" ? "default" : "outline"} onClick={() => {
              ...
              <Button variant={locationMode === "pincode" ? "default" : "outline"} onClick={() => setLocationMode("pincode")} className="gap-2">
                Search by Pincode
              </Button>
              <Button variant={locationMode === "all" ? "default" : "outline"} onClick={() => setLocationMode("all")}>
                Show All
              </Button>
            </div>

            {locationMode === "pincode" && <Input type="text" placeholder="Enter your pincode..." value={userPincode} onChange={e => setUserPincode(e.target.value)} className="h-12 text-center" />}
          </div>
        </div>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          {loading ? <div className="grid gap-6 md:grid-cols-2">
            ...
            </div> : filteredHospitals.length === 0 ? <Card className="text-center py-12">
              <CardContent>
            ...
            return <Card key={hospital.id} className="hover:shadow-soft-lg transition-all duration-300 border-primary/10 hover:border-primary/30 cursor-pointer" onClick={() => navigate(`/hospital/${hospital.id}`)}>
                    <CardHeader>
                    ...
                    <CardContent>
                      ...
                      {hospital.waiting_lists?.[0]?.last_updated && <p className="text-xs text-muted-foreground mt-3 flex items-center gap-1">
